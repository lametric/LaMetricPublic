...
# >>>> Delete if you hard coded your client_id and client_secret below
import config 
# <<<<<
...
class LaMetricCloudException (LaMetricException):
    ...
    def __str__(self):
        if self.status == 0:
            ...
            return 'LaMetric Cloud Error. {}: {}'.format(self.status,
                                                         self.message)
...
class LaMetricDeviceException (LaMetricException):
    ...
    def __str__(self):
        if self.status == 0:
            ...
            return 'LaMetric Device Error. {}: {}'.format(self.status,
                                                          self.message)
...
class LaMetricCloudClient:
    ...
    def authenticate(self, code):
        ...
        body = {'grant_type': 'authorization_code',
                ...
                'code': code}
        try:
            res, content = self._http.request(
                TOKEN_URL, "POST", urlencode(body), headers)
            parsed = json.loads(content.decode('utf-8'))
            if res.status == 200:
                self._access_token = parsed['access_token']
                self._refresh_token = parsed['refresh_token']
            else:
                raise LaMetricCloudException(
                    parsed['errors'][0]['message'], res.status)
        except httplib2.ServerNotFoundError:
            raise LaMetricCloudException("Host is not reachable")
        except JSONDecodeError:
            raise LaMetricCloudException(
                "Authenticate. Unknown response format.")
    ...
    def refresh_token(self):
        if self._refresh_token != None:
            ...
            try:
                ...
                raise LaMetricCloudException(
                    "Refresh token. Unknown response format.")
    ...
    def get_devices(self):
        ...
        try:
            ...
            if res.status == 200:
                ...
                raise LaMetricCloudException(
                    devices['errors'][0]['message'], res.status)
        except httplib2.ServerNotFoundError:
            raise LaMetricCloudException("Host is not reachable")
        except JSONDecodeError:
...
def run_example():
    ...
    try:
        if lametric_client.has_refresh_token():
            ...
            print('Insert this URL into browser: {}'.format(
                lametric_client.authentication_url()))
        ...
        while True:
            icon = input(
                "Enter icon code or hit enter (find code on http://developer.lametric.com/icons) [a2867] ---> ")
            if icon == 'exit':
                break
            if icon == '':
                icon = 'a2867'
